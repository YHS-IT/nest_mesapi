...
@Entity("process_report_temp" ,{schema:"data" } )
export  class ProcessReportTemp {

@Column("date",{ name:"report_date",comment:"리포트 집계일" })
reportDate:string;

@Column("int",{ name:"ent_id",comment:"업체ID" })
entId:number;

@Column("int",{ name:"mkey",comment:"기계 고유ID (svc_cnc.id)" })
mkey:number;

@Column("int",{ name:"lot",comment:"생산로트" })
lot:number;
